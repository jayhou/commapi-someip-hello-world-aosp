...
void callbackPass(std::function<void(const CommonAPI::AvailabilityStatus&)> callback) {
    std::cout << "callbackPass !!!" << std::endl;
    if(callback) {
        std::cout << "valid callback !" << std::endl;
        callback(CommonAPI::AvailabilityStatus::UNKNOWN);
    } else {
...
int main() {
    ...
    callbackPass(proxyStatusListener);
